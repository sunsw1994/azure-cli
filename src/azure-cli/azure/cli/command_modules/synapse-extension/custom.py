...
from azure.synapse.spark.models import SparkBatchJobOptions, SparkSessionOptions
...
def create_spark_batch_job(cmd, client, workspace_name, spark_pool_name, job_name, main_definition_file,
                           main_class_name, executor_size, executor_count, language=SparkBatchLanguage.Scala,
...
def get_spark_batch_job(cmd, client, job_id):
    return client.get(job_id, detailed=True)


def cancel_spark_batch_job(cmd, client, job_id):
    return client.delete(job_id)
...
def create_spark_session_job(cmd, client, workspace_name, spark_pool_name, job_name, executor_size, executor_count,
